  ...
  <section class="content">
    ...
    <table class="table ">
        ...
        <?php
            $order_data =array();
            $counter = 0;
            $order_data = $db->displayDispatchedOrders();

           
            foreach($order_data as $record)
            {
                $user_full_name    =$order_data[$counter]['user_full_name'];
                $mobile_no         =$order_data[$counter]['mobile_no'];
                $address           =$order_data[$counter]['address'];
                $city              =$order_data[$counter]['city'];
                $zipcode           =$order_data[$counter]['zipcode'];
                $total             =$order_data[$counter]['total'];
                $status            =$order_data[$counter]['status'];
                $date              =$order_data[$counter]['date'];
                $time              =$order_data[$counter]['time'];
